...
- groundtruth/：manifest.json（漏洞标签）+ check_manifest.py / score_output.py / compare_src_ir.py / extract_ir_records.py。
...
1. 指令覆盖验证：用 DevEco SDK 的 ark_disasm（`$DEVECO_SDK_HOME/default/openharmony/toolchains/ark_disasm`）
   ...
   （release 构建跑一次落快照，debug 构建再跑即 release∪debug 并集；全集取逆向工具链仓库的 isa.yaml，
   路径经环境变量 ISA_YAML 或 --isa-yaml 传入；
   当前 release 169/267、并集 172/267，剩余均为实测不可达或 deprecated，清单见 docs/BENCHMARK.md）。
2. 评分：由逆向工具产出的 test.out 作为输入，`python3 groundtruth/score_output.py test.out build/outputs/default/*.app`。
